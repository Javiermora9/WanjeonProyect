...
<html lang="en">
<head>
    <title>Code coverage report for src/ciphers/eme/index.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../index.html">All files</a> / <a href="index.html">src/ciphers/eme</a> index.js
    </h1>
    <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">// Javascript port of https://github.com/rfjakob/eme/blob/master/eme.go by rfjakob
&nbsp;
...
function multByTwo(out, input) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (input.length !== 16) {
<span class="cstat-no" title="statement not covered" >    throw new Error('Invalid length');</span>
  }
  let last = input[0];
&nbsp;
  out[0] = (2 * input[0]) &amp; 0xff;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (input[15] &gt;= 128) {
    out[0] ^= 135;
  }
  for (let j = 1; j &lt; 16; j++) {
    const tmp = input[j];
...
function xorBlocks(out, in1, in2) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (in1.length !== in2.length &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >in2.length !== out.length)</span> {
<span class="cstat-no" title="statement not covered" >    throw new Error('Length must all match');</span>
  }
&nbsp;
...
function aesTransform(src, direction, bc) {
  ...
  } else <span class="missing-if-branch" title="else path not taken" >E</span>if (direction === DirectionDecrypt) {
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (T.length !== 16) {
<span class="cstat-no" title="statement not covered" >    throw new Error('Tweak must be 16 bytes long');</span>
  }
  <span class="missing-if-branch" title="if path not taken" >I</span>if (P.length % 16 !== 0) {
<span class="cstat-no" title="statement not covered" >    throw new Error('Input Data must be a multiple of 16 long');</span>
  }
&nbsp;
  const m = P.length / 16;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (m === 0 || m &gt; 128) {
...
&nbsp;
  ...
  for (let j = 1; j &lt; m; j++) {
<span class="cstat-no" title="statement not covered" >    xorBlocks(MP, MP, C.subarray(j * 16, (j + 1) * 16));</span>
  }
  /* MC = AESenc(K; MP) */
...
&nbsp;
  ...
  for (let j = 1; j &lt; m; j++) {
<span class="cstat-no" title="statement not covered" >    multByTwo(M, M);</span>
    /* CCCj = 2**(j-1)*M xor PPPj */
<span class="cstat-no" title="statement not covered" >    xorBlocks(CCCj, C.subarray(j * 16, (j + 1) * 16), M);</span>
<span class="cstat-no" title="statement not covered" >    C.set(CCCj, j * 16);</span>
  }
&nbsp;
  ...
  xorBlocks(CCC1, MC, T);
  for (let j = 1; j &lt; m; j++) {
<span class="cstat-no" title="statement not covered" >    xorBlocks(CCC1, CCC1, C.subarray(j * 16, (j + 1) * 16));</span>
  }
  C.set(CCC1, 0);
&nbsp;
  for (let j = 0; j &lt; m; j++) {
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="https://istanbul.js.org/" target="_blank">istanbul</a> at Thu Apr 16 2020 08:36:55 GMT+0000 (Coordinated Universal Time)
</div>
</div>
